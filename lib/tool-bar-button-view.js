...
export default class ToolBarButtonView {
  ...
  constructor (options, group) {
    ...
    if (options.tooltip) {
      ...
      this.label.innerHTML = options.tooltip;
    }
    ...
    if (options.iconset) {
      classNamesIcon.push(options.iconset, `${options.iconset}-${options.icon}`);
    } else {
      classNamesIcon.push(`icon-${options.icon}`);
    }
    this.icon.classList.add(...classNamesIcon);
